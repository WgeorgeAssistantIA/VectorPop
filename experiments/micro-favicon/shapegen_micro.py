...
def extract_shapes(arr, n_colors: int, n_shapes: int, min_area_frac: float = 0.008):
    ...
    d = np.linalg.norm(
        arr[..., None, :] - centers[None, None, :, :], axis=-1
    )  # H,W,n_colors
...
def render_svg(blobs, canvas_hw, size_px: int) -> str:
    ...
    for b in blobs:
        ...
        if kind == "circle":
            ...
            rx = min(w, h) * 0.22
            parts.append(
                f'<rect x="{x:.2f}" y="{y:.2f}" width="{w:.2f}" height="{h:.2f}" rx="{rx:.2f}" fill="{col}"/>'
            )
        else:
            w = (b["x1"] - b["x0"] + 1) * scale / 2
            h = (b["y1"] - b["y0"] + 1) * scale / 2
            parts.append(
                f'<ellipse cx="{cx:.2f}" cy="{cy:.2f}" rx="{w:.2f}" ry="{h:.2f}" fill="{b["color"] and "#%02x%02x%02x" % b["color"]}"/>'
            )
    parts.append("</svg>")
    return "\n".join(parts)
...
def main():
    ...
    for size in (32, 16):
        ...
        for n_colors, n_shapes, label_ in configs:
            ...
            (OUT_DIR / f"shapegen_{size}px_{n_shapes}formes.svg").write_text(
                svg, encoding="utf-8"
            )
