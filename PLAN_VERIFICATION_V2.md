...
| L5 | Panneau SVG : zoom maximal | ~×12 000 (28 crans à ×1,4/cran, relevé de ×1,25 après retour utilisateur "pas l'impression que ce soit x10000"), pastille de zoom persistante sur le panneau dès les premiers crans (pas juste au max), cache bitmap coupé au-delà de ×8, indicateur « Zoom ×… » dans la barre d'état, rendu rapide | ✅ |
...
| 24/09/2026 | Retour William : le zoom ×10000 « ne se sentait pas » | William confirme le fondu et la pastille Pro OK, mais signale que le zoom du panneau SVG ne donne pas l'impression d'atteindre x10000. Diagnostic : le calcul était correct (x9400 mesuré), mais 41 crans de molette à ×1,25 chacun, c'est long à scroller, et une courbe vectorielle ne pixellise jamais -- au bout d'un moment, zoomer davantage ne montre rien de visiblement différent. Corrigé : pas relevé à ×1,4 (28 crans, ~×12 000), et une pastille de zoom reste affichée sur le panneau tant qu'on n'est pas revenu à "ajusté" (avant, seul un message de la barre d'état, qui disparaissait en 1,5s). 66/66 PASS. |
| 24/09/2026 | Packaging & Release V2.0.0 | Bump 2.0.0 sur tous les composants. Compilation validée : Inno Setup EXE, MSIX Windows Store, ZIP portable, AppImage Linux, Tar.gz Linux, Snap Linux. Tests 66/66 PASS. Publication GitHub Releases v2.0.0 avec 6 binaires, publication Snap Store (rév. 4 stable), soumission Partner Center (MSIX), déploiement site web (version.json 2.0.0 et freemium 2.0). Tout est vert et déployé. |
